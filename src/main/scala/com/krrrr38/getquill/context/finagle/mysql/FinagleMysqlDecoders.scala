...
trait FinagleMysqlDecoders {
  ...
  case class FinagleMysqlDecoder[T](decoder: DecoderMethod[T])
  ...
  def decoder[T: ClassTag](f: PartialFunction[Value, T]): Decoder[T] =
    FinagleMysqlDecoder((index, row, session) => {
      ...
      f.lift(value)
        .getOrElse(
          fail(
            s"Value '$value' can't be decoded to '${classTag[T].runtimeClass}'"
          )
        )
    })
  ...
  implicit def mappedDecoder[I, O](implicit
    ...
    FinagleMysqlDecoder(mappedBaseDecoder(mapped, decoder.decoder))

  implicit val stringDecoder: Decoder[String] =
    decoder[String] { case StringValue(v) =>
      v
    }
  implicit val bigDecimalDecoder: Decoder[BigDecimal] =
    decoder[BigDecimal] { case BigDecimalValue(v) =>
      v
    }
  ...
  implicit val shortDecoder: Decoder[Short] =
    decoder[Short] { case ShortValue(v) =>
      v
    }
  ...
  implicit val floatDecoder: Decoder[Float] =
    decoder[Float] { case FloatValue(v) =>
      v
    }
  implicit val doubleDecoder: Decoder[Double] =
    decoder[Double] { case DoubleValue(v) =>
      v
    }
  implicit val byteArrayDecoder: Decoder[Array[Byte]] =
    decoder[Array[Byte]] { case v: RawValue =>
      v.bytes
    }
  ...
  implicit val localDateTimeDecoder: Decoder[LocalDateTime] =
    decoder[LocalDateTime] { case `timestampValue`(v) =>
      v.toInstant.atZone(extractionTimeZone.toZoneId).toLocalDateTime
    }

  implicit val uuidDecoder: Decoder[UUID] =
    mappedDecoder(MappedEncoding(UUID.fromString), stringDecoder)
}
