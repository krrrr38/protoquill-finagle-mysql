...
case class FinagleMysqlContextConfig(config: Config) {
  ...
  def injectionTimeZone = Try(
    TimeZone.getTimeZone(config.getString("timezone.injection"))
  ).getOrElse(TimeZone.getDefault)
  def extractionTimeZone = Try(
    TimeZone.getTimeZone(config.getString("timezone.extraction"))
  ).getOrElse(TimeZone.getDefault)
  ...
  def maxPrepareStatements =
    Try(config.getInt("maxPrepareStatements")).getOrElse(20)
  ...
  def connectionInitSql =
    Try(Some(config.getString("connectionInitSql"))).getOrElse(None)
